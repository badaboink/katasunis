...
export default function ViewProfiles() {
  ...
  const getUsers = () => {
    fetch(
      "http://localhost/katasunis/katasunis_backend/perziureti_naudotojus.php"
    )
  ...
  const DeleteProfile = (event, id) => {
    ...
    if (event.target.innerHTML === "Šalinti") {
      ...
      fetch(
        `http://localhost/katasunis/katasunis_backend/istrinti_naudotojus.php?id=${id}`,
        {
          method: "DELETE",
          headers: {
      ...
      <ul class="responsive-table">
        ...
        {users.map((user) => (
          <li class="table-row">
            ...
            <div
              class="col col-4"
              onClick={(event) => DeleteProfile(event, user.id)}
              data-label="Salinti"
            >
